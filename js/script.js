...
$('#top-silder .owl-carousel').owlCarousel({
   loop:true,
   margin:30,
   nav:false,
   dots:true,
...
/*
document.querySelectorAll('#v-pills-tab-menu .nav-link').forEach(function(navLink) {
       navLink.addEventListener('mouseover', function() {
           document.querySelectorAll('.tab-pane').forEach(function(tabPane) {
               tabPane.classList.remove('show', 'active');
           });
           document.querySelectorAll('#v-pills-tab-menu .nav-link').forEach(function(link) {
               link.classList.remove('active');
           });
           let tabTrigger = new bootstrap.Tab(navLink);
           tabTrigger.show();
